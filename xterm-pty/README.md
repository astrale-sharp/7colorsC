...
You can compile your application with [`-s PROXY_TO_PTHREAD`](https://emscripten.org/docs/porting/pthreads.html?highlight=proxy_to_pthread#additional-flags). In this mode Emscripten will transparently move your application to run in a pthread (in a Web Worker).
...
The integration hack highly depends on the internal implementation of the Emscripten runtime. It works with the upcoming 3.1.47, which for now can be installed via `emsdk install tot`. (TODO: update this when 3.1.47 is released)
